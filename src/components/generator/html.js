...
const layouts = {
  ...
  tableItem(scheme) {
    ...
    const required = !ruleTrigger[config.tag] && config.required ? 'required' : ''
    const children = config.children.map(el => layouts[el.__config__.layout](el))
    const str = `<el-table ${labelWidth} ${label} data="${config.dataPath}" ${required}>
        ${children.join('\n')}
      </el-table>`
    return str
  },
  ...
  raw(scheme) {
    let children = []
    if (scheme.__config__.children !== undefined) {
      children = scheme.__config__.children.map(el => tags[el.__config__.tag](el))
    }
    const str = `<el-table-column label = '${scheme.label}'>
        ${children.join('\n')}
      </el-table-column>`
    return str
  }
...
const tags = {
  ...
  'el-table': el => {
    ...
    // const { tag } = el.__config__
    const props = el.__config__.props ? `props="${el.__config__.__vModel__}Props"` : ''
    const label = el.__config__.label ? `label="${el.__config__.label}"` : ''
